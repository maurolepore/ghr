...
The ghr package helps you to request information to the GitHub-API using an intuitive syntax, similar to that of `remotes::install_github()`. For example, this is a valid path `maurolepore/ghr@master`. Also this one: `maurolepore/ghr/reference@gh-pages`.
...
Use `ghr_ls()` as a shortcut for `ghr_pull(ghr_get(path), field = "path"). It offers an interface similar to `fs::dir_ls()`.
